...
def main():
    ...
    for idx in range(num_worker):
        ...
        work = env_type(env_id, is_render, idx, child_conn, writer)
        work.start()
        works.append(work)
    ...
    while True:
            ...
            for _ in range(num_step):
                actions, value, policy = agent.GetAction((np.float32(states) - obs_rms.mean) / np.sqrt(obs_rms.var))
            ...
            _, value, _ = agent.GetAction((states - obs_rms.mean) / np.sqrt(obs_rms.var))
            total_values.append(value)
            # --------------------------------------------------
