...
def main():
    ...
    icm_path = 'models/{}/{}.icm'.format(env_id, env_id)
    ...
    while True:
        ...
        for _ in range(num_step):
            actions, value_ext, value_int, policy = agent.GetAction((np.float32(states) - obs_rms.mean) / np.sqrt(obs_rms.var))
            ...
            intrinsic_reward = agent.ComputeIntrinsicReward(
        ...
        #target = int_target * int_coef  + ext_target * ext_coef
        adv = int_adv * int_coef + ext_adv * ext_coef
        

        # -----------------------------------------------
        # Step 4. Training!
        #print("Training")
        agent.TrainModel((total_state - obs_rms.mean) / np.sqrt(obs_rms.var),
