...
def main():
    ...
    model_path = 'models/Intrinsic_Reward_Only/{}.model'.format(env_id)
    predictor_path = 'models/Intrinsic_Reward_Only/{}.pred'.format(env_id)
    target_path = 'models/Intrinsic_Reward_Only/{}.target'.format(env_id)
    
    if default_config['TrainMethod'] == 'RND':
        #Instantiate the agent
        print("Using RND agent")
        agent = RNDAgent(
            input_size,
            output_size,
            num_worker,
    ...
    elif default_config['TrainMethod'] == 'ICM':
        print("Using ICM agent")
        agent = ICM(
            input_size,
            output_size,
            num_worker,
    ...
    elif default_config['EnvType'] == 'loz':
    ...
    while True:
        ...
        for _ in range(num_step):
            ...
            intrinsic_reward = agent.ComputeIntrinisicReward(
        ...
        _, value_ext, value_int, _ = agent.GetAction(np.float32(states) / 255.)
        total_ext_values.append(value_ext)
        total_int_values.append(value_int)
        # --------------------------------------------------
        ...
        # Step 5. Training
        agent.TrainModel(np.float32(total_state) / 255., ext_target, int_target, total_action,
