...
def main():
    ...
    model_path = 'models/IntrinsicReward/{}/{}.model'.format(env_id, env_id)
    predictor_path = 'models/IntrinsicReward/{}/{}.pred'.format(env_id, env_id)
    target_path = 'models/IntrinsicReward/{}/{}.target'.format(env_id, env_id)
    

    #transfer_model_path = 'models/Transfer/SuperMarioBros-1-1-v0/SuperMarioBros-1-1-v0.model'
    #transfer_predictor_path = 'models/Transfer/SuperMarioBros-1-1-v0/SuperMarioBros-1-1-v0.pred'

    # Set the writer (Tensorboard)
    writer = SummaryWriter("new_runs/intONLY_rewards_" + env_id)
   ...
   # Loads models
    if is_load_model:
        if use_cuda:
            ...
            agent.model.load_state_dict(torch.load(transfer_model_path))

            #print("Loading RND Predictor Saved Model using GPU")
            #agent.rnd.predictor.load_state_dict(torch.load(transfer_target_path), strict=False)

            #print("Loading RND Target Saved Model using GPU")
            #agent.rnd.target.load_state_dict(torch.load(transfer_predictor_path), strict=False)
        else:
    ...
    for idx in range(num_worker):
        ...
        work = env_type(env_id, writer, is_render, idx, child_conn, sticky_action=sticky_action, p=action_prob,
                        life_done=life_done)
        work.start()
        works.append(work)
    ...
    while True:
        ...
        for _ in range(num_step):
            ...
            if real_dones[sample_env_idx]:
                ...
                #writer.add_scalar('data/step', sample_rall, info['x_pos'])
                sample_rall = 0
                sample_step = 0
                sample_i_rall = 0
